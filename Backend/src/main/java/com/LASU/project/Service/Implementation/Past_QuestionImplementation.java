...
public class Past_QuestionImplementation implements Past_QuestionService {
    ...
    @Override
    public List<Past_Question> searchPastQuestion(String keyword) throws IOException {
        return questionRepository.findByPastQuestion(keyword);

    }

    @Override
    public List<Past_Question> findBySchoolId(String keyword) throws IOException {
        return questionRepository.findBySchoolId(keyword);

    }

    @Override
    public void deleteById(Long id) throws GeneralException, IOException {

        Past_Question response = questionRepository.findById(id).orElseThrow(
                () -> new GeneralException("Past Question with Id "+ id +" Not Found"));

        cloudinaryService.deleteImage(response.getFile());
