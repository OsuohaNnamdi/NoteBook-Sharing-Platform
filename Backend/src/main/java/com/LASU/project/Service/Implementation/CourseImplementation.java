...
public class CourseImplementation implements CourseService {
    ...
    @Override
    public void updateCourse(Long id, Course request, MultipartFile document) throws GeneralException, IOException {
        ...
        try {
            ...
            if (course != null){
                ...
                if (document != null) {
                    cloudinaryService.deleteImage(course.getFile());
                    String response = cloudinaryService.uploadImage(document);
                    course.setFile(response);
                }
                courseRepository.save(course);
